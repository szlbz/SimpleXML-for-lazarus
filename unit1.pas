...
uses
  Classes, SysUtils, Forms, Controls, Graphics, Dialogs, StdCtrls,SimpleJSON,SimpleXML;
...
var
  ...
  value: string;
begin
  // 使用全局函数 ParseXML 代替 TXmlNode.Parse
  xml := ParseXML('<root><name>张三</name><age>25</age></root>');

  value := xml.GetS('name'); // 返回 "张三"
  ShowMessage(value);
  xml.SetI('age', 30);
  xml.SaveToFile('output.xml');
  Memo1.Lines.LoadFromFile('output.xml');
...
begin
  ...
  Memo1.Lines.LoadFromFile('Created.xml');
end;
